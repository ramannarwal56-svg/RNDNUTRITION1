...
code = code.replace(
  /const cleanEmail = emailInput\.replace\(\/\\D\/g, ''\)\.slice\(-10\);\s*if \(cleanEmail\.length !== 10\) \{\s*showToast\("Please enter a valid 10-digit phone number", "warning"\);\s*return;\s*\}/,
...
code = code.replace(/id="account-phone"/g, 'id="account-email" type="email"');
...
code = code.replace(/user\.phone/g, "user.email");
